...
export const FeaturedContentOptions = ({ attributes, setAttributes }) => {
		...
		return (
			<>
				<Notice
					...
					isDismissible={false}
				>
					{__('These options can only be used in a single post. Keep in mind that, by selecting this option, the editor preview may not show the correct items!', 'esseker')}
				</Notice>
				<br />
			</>
		);
	...
	return (
		<PanelBody title={__('Featured content', 'esseker')}>
			{postTypeOptions.length > 0 &&
				<>
					<CustomSelect
						label={<IconLabel icon={icons.file} label={__('Filter by content type', 'esseker')} />}
					...
					{taxonomyOption().length > 0 &&
						<CustomSelect
							label={<IconLabel icon={icons.editOptions} label={__('Filter by taxonomy', 'esseker')} />}
					...
					{featuredContentTaxonomy &&
						<>
							<IconToggle
								...
								label={__('Select specific items', 'esseker')}
								help={__('These options will allow you to manually select items to show.', 'esseker')}
							...
							{useSpecificTerms ?
								<CustomSelect
									label={<IconLabel icon={icons.clipboard}  label={__('Show only specific items', 'esseker')} />}
									help={__('You will preview the latest 30 items, use the search option to find the item you are looking for. If nothing is selected, all items are shown.', 'esseker')}
								...
								<>
									<IconToggle
										...
										label={__('Use term of the current post', 'esseker')}
										help={__('This option allows you to show items in the current post taxonomy. In order for this to work you must select value in the Filter by taxonomy option.', 'esseker')}
					...
					{!featuredContentTaxonomy &&
						<>
							<IconToggle
								icon={icons.visible}
								label={__('Select specific items to show?', 'esseker')}
								checked={useSpecificPosts}
								help={__('This option allows you to select individual items to show.', 'esseker')}
								onChange={() => {
							...
							{useSpecificPosts &&
								<CustomSelect
									label={<IconLabel icon={<BlockIcon iconName='esf-select' />}  label={__('Show only specific items', 'esseker')} />}
									help={__('You will preview the latest 30 items, use the search option to find the item you are looking for. If nothing is selected, all items are shown.', 'esseker')}
			...
			<IconToggle
				icon={icons.none}
				label={__('Exclude the current post', 'esseker')}
				checked={featuredContentExcludeCurrentPost}
				help={__('Exclude the current post from the featured content block items.', 'esseker')}
			...
			<IconToggle
				...
				label={__('Use random order', 'esseker')}
				checked={featuredContentRandomOrder}
				help={__('Show posts in random order.', 'esseker')}
