...
if ($featuredContentPostType === 'books-api') {
	$data = apply_filters(BooksData::BOOKS_DATA, []);
} else {
...
>
	...
	$cards = Components::renderPartial(
		...
		'cards',
		[
			'items' => $featuredContentPostType === 'books-api' ? $data : $mainQuery->posts, // @phpstan-ignore-line
	...
	if (!$featuredContentServerSideRender) {
		echo Components::render(
			...
			Components::props('loadMore', $attributes, [
				'loadMoreInitiaItems' => wp_json_encode($mainQuery->posts), // @phpstan-ignore-line
