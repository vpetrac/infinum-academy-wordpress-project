...
export const ColumnOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Column', 'esseker')}>

			<HelpModal
				type='column'
				buttonLabel={__('How to use a Column?', 'esseker')}
				modalLabel={__('Column', 'esseker')}
			/>
			...
			<Responsive label={<IconLabel icon={icons.order} label={__('Order', 'esseker')} />}>
				{breakpoints.map((keyName, index) => {
					...
					return (
						<Fragment key={keyName}>
							...
							/>
							{index === 0 &&
								<p className='es-decorative-text'>{__('Needs to be set on all columns!', 'esseker')}</p>
							}
						</Fragment>
					);
				})}
			...
			<Responsive label={<IconLabel icon={icons.verticalAlign} label={__('Vertical align', 'esseker')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
			...
			<Responsive label={<IconLabel icon={icons.horizontalAlign} label={__('Horizontal align', 'esseker')} />}>
				{breakpoints.map((keyName, index) => {
					const point = ucfirst(keyName);
