...
export const ColumnsOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Columns', 'esseker')}>
			<HelpModal
				type="columns"
				buttonLabel={__('How to use Columns?', 'esseker')}
				modalLabel={__('Columns', 'esseker')}
			/>

			<br /><br />

			<Responsive label={<IconLabel icon={icons.gutter} label={__('Column spacing', 'esseker')} />}>
			...
			<Responsive label={<IconLabel icon={icons.verticalSpacing} label={__('Row spacing', 'esseker')} />}>
				{breakpoints.map((keyName) => {
					const point = ucfirst(keyName);
