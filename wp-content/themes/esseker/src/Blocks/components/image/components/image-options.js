	...
	return (
		<>
			...
			{imageUse &&
				<>
					{showImageUrl &&
						<Responsive label={<IconLabel icon={icons.image} label={__('Image source', 'esseker')} />}>
							{Object.keys(checkAttrResponsive('imageUrl', attributes, manifest)).map(function (keyName) {
								...
								if (point === 'Large') {
									point = '';
									pointLabel = __('Default (all screens)', 'esseker');
								}
					...
					{showImageAlt &&
						<TextareaControl
							label={<IconLabel icon={icons.altText} label={__('Alt text', 'esseker')} />}
							value={imageAlt}
					...
					{showImageFull &&
						<IconToggle
							icon={icons.size}
							label={__('Fill container', 'esseker')}
							help={__('Span the full width of the container and ignore its maximum width', 'esseker')}
							checked={imageFull}
