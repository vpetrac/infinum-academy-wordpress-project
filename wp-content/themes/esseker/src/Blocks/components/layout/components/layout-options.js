...
export const LayoutOptions = (attributes) => {
	...
	return (
		<CollapsableComponentUseToggle
			label={sprintf(__('%s', 'esseker'), label)}
			checked={layoutUse}
		...
		>
			<>
				{showLayoutType &&
					<SimpleVerticalSingleSelect
						label={<IconLabel icon={icons.backgroundTypeAlt} label={__('Layout Type', 'esseker')} />}
				...
				{showLayoutTotalItems &&
					<RangeControl
						label={<IconLabel icon={icons.totalItems} label={__('Maximum number of items to show', 'esseker')} />}
