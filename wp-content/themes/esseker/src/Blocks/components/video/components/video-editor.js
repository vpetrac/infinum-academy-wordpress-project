		...
		<>
			{!hasVideo && showVideoUrl &&
				<MediaPlaceholder
					...
					labels={{
						title: __('Video', 'esseker'),
						instructions: __('Upload a video file or pick one from your media library. You can select multiple video files to have fallbacks with different video formats.', 'esseker'),
					}}
					multiple
					accept={videoAccept}
			...
			{!hasVideo && !showVideoUrl && showPlaceholder &&
				<Placeholder icon={icons.video} label={__('Add a video', 'esseker')}>
					{__('Check the block options in the sidebar', 'esseker')}
				</Placeholder>
			}
