			...
			{buttonUse &&
				<>
					...
					{showButtonIsLink &&
						<IconToggle
							icon={icons.link}
							label={__('Display as link', 'esseker')}
					...
					{showButtonSize && !buttonIsLink &&
						<SimpleVerticalSingleSelect
							label={<IconLabel icon={icons.size} label={__('Size', 'esseker')} />}
							options={sizeOptions}
						/>
					}
					...
					{showButtonWidth && !buttonIsLink &&
						<SimpleVerticalSingleSelect
							label={<IconLabel icon={icons.width} label={__('Width', 'esseker')} />}
					...
					{showButtonAriaLabel &&
						<TextControl
							label={<IconLabel icon={icons.infoCircle} label={__('ARIA label', 'esseker')} />}
					...
					{showButtonIsAnchor &&
						<IconToggle
							icon={icons.anchor}
							label={__('Anchor', 'esseker')}
							checked={buttonIsAnchor}
							onChange={(value) => setAttributes({ [getAttrKey('buttonIsAnchor', attributes, manifest)]: value })}
							help={__('Make sure to set the URL as an ID of the element you want to target, e.g. #my-block.', 'esseker')}
						/>
					}

					{showButtonId &&
						<TextControl
							label={<IconLabel icon={icons.id} label={__('Unique identifier', 'esseker')} />}
							value={buttonId}
